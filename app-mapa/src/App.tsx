...
function ProjectMarker({
  ...
  return (
    <Marker position={position} icon={icon} eventHandlers={{ click: onSelect }}>
      {isSelected && (
        <Popup
          closeButton={false}
          autoPan={true}
...
function PopupContent({ obra }: { obra: PublicObra }) {
  ...
  return (
    <div style={{ width: 300, color: 'var(--text-primary)' }}>
      <div className="px-1 pt-1 pb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2 mb-1">
          <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: statusColor, boxShadow: `0 0 6px ${statusColor}80` }} />
      ...
      <div className="py-2 space-y-2">
        ...
        <div>
          ...
          <div className="h-1.5 rounded-full overflow-hidden ml-4" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${obra.avanceFisico}%`, background: statusColor }} />
          </div>
        </div>
      ...
      <div className="pt-2 flex items-center justify-between" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <span className="text-[9px]" style={{ color: 'var(--text-muted)' }}>{obra.totalInformes} informe{obra.totalInformes !== 1 ? 's' : ''}</span>
        <span className="text-[9px] truncate max-w-[120px]" style={{ color: 'var(--text-muted)' }}>{obra.supervisorNombre}</span>
      </div>
    </div>
  );
}
    ...
    <div className="fixed top-0 left-0 right-0 z-[1000] pointer-events-none">
      <div className="glass-panel mx-3 mt-3 px-5 py-3 flex items-center justify-between pointer-events-auto" style={{ borderRadius: 16 }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[13px] font-bold" style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>Mapa Inteligente — Obras Publicas</h1>
            <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Temascaltepec de Gonzalez, Edo. Mex.</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-[12px] font-semibold" style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>{time.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>
          <div className="text-[9px] uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>{time.toLocaleDateString('es-MX', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</div>
        </div>
      </div>
    </div>
  );
...
function HudCorners() {
  const s: React.CSSProperties = { position: 'fixed', width: 24, height: 24, borderColor: 'rgba(59,130,246,0.25)', zIndex: 999, pointerEvents: 'none' };
  return (
    <>
      <div style={{ ...s, top: 12, left: 12, borderTop: '2px solid', borderLeft: '2px solid' }} />
      <div style={{ ...s, top: 12, right: 12, borderTop: '2px solid', borderRight: '2px solid' }} />
      <div style={{ ...s, bottom: 12, left: 12, borderBottom: '2px solid', borderLeft: '2px solid' }} />
      <div style={{ ...s, bottom: 12, right: 12, borderBottom: '2px solid', borderRight: '2px solid' }} />
    </>
  );
}
...
function ScanLine() {
  return (
    <div className="fixed inset-0 pointer-events-none z-[998]" style={{ background: 'linear-gradient(to bottom, transparent 50%, rgba(59,130,246,0.015) 50%)', backgroundSize: '100% 4px' }} />
  );
}
...
function KpiPanel({ obras }: { obras: PublicObra[] }) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[1000] flex gap-2">
      {kpis.map((kpi, i) => (
        <motion.div key={kpi.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08, duration: 0.4 }} className="glass-panel px-4 py-2.5 flex flex-col items-center" style={{ borderRadius: 12, minWidth: 80 }}>
          <span className="text-[15px] font-bold" style={{ fontFamily: 'var(--font-display)', color: kpi.color }}>{kpi.isText ? kpi.value : kpi.value.toLocaleString('es-MX')}</span>
          <span className="text-[8px] uppercase tracking-wider mt-0.5" style={{ color: 'var(--text-muted)' }}>{kpi.label}</span>
        </motion.div>
      ))}
    </div>
  );
...
function SmartMap({ obras }: { obras: PublicObra[] }) {
  ...
  const createClusterCustomIcon = useCallback((cluster: any): L.DivIcon => {
    ...
    return L.divIcon({
      ...
      html: `
        <div style="width: ${size}px; height: ${size}px; border-radius: 50%; background: rgba(59,130,246,0.18); border: 2px solid rgba(59,130,246,0.45); box-shadow: 0 0 16px rgba(59,130,246,0.2); display: flex; align-items: center; justify-content: center; color: white; font-family: 'Syne', sans-serif; font-weight: 700; font-size: ${Math.max(11, size * 0.3)}px;">${count}</div>
      `,
    });
  }, []);
  ...
  return (
    <div className="absolute inset-0 z-0">
      <MapContainer center={TEMASCALTEPEC_CENTER} zoom={DEFAULT_ZOOM} minZoom={MIN_ZOOM} maxZoom={MAX_ZOOM} style={{ width: '100%', height: '100%', background: '#080c0f' }} zoomControl={true} attributionControl={true}>
        <TileLayer
        ...
        <MarkerClusterGroup showCoverageOnHover={false} spiderLegPolylineOptions={{ opacity: 0 }} iconCreateFunction={createClusterCustomIcon} maxClusterRadius={60}>
          {obras.map((obra) => (
            <ProjectMarker key={obra.id} obra={obra} isSelected={selectedObra?.id === obra.id} onSelect={() => setSelectedObra(prev => prev?.id === obra.id ? null : obra)} />
          ))}
        </MarkerClusterGroup>
      </MapContainer>
    </div>
...
function ErrorScreen({ error, onRetry }: { error: string; onRetry: () => void }) {
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center px-6" style={{ background: '#080c0f' }}>
      ...
      <h2 className="text-lg font-bold mb-2" style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>Error de conexion</h2>
      <p className="text-sm text-center mb-6" style={{ color: 'var(--text-secondary)' }}>No se pudo cargar la informacion del mapa. Verifica tu conexion a internet.</p>
      <p className="text-[10px] mb-6 text-center" style={{ color: 'var(--text-muted)' }}>{error}</p>
      <button onClick={onRetry} className="px-6 py-2.5 rounded-lg text-sm font-medium transition-all" style={{ background: 'rgba(59,130,246,0.15)', color: '#3b82f6', border: '1px solid rgba(59,130,246,0.3)' }}>Reintentar</button>
    </div>
  );
}
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [obrasData] = await Promise.all([fetchObrasPublic(), fetchResumenPublic().catch(() => null)]);
      setObras(obrasData);
    } catch (err: any) {
